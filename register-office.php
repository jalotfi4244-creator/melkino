...
if (($_SERVER['REQUEST_METHOD'] ?? '') === 'POST' && isset($_POST['submit_property'])) {
    ...
    // گام ۲: مشخصات اجباری واحد اداری
    if (empty($_POST['office_area'])) $errors[] = 'لطفاً متراژ واحد را وارد کنید.';
    if (empty($_POST['office_floor'])) $errors[] = 'لطفاً طبقه را وارد کنید.';
    if (empty($_POST['office_units_per_floor'])) $errors[] = 'لطفاً تعداد واحد در طبقه را وارد کنید.';
    if (empty($_POST['office_rooms'])) $errors[] = 'لطفاً تعداد اتاق را انتخاب کنید.';
    if (empty($_POST['office_year'])) $errors[] = 'لطفاً سال ساخت را وارد کنید.';
    if (empty($_POST['office_condition'])) $errors[] = 'لطفاً وضعیت واحد را انتخاب کنید.';
    if (empty($_POST['office_orientation'])) $errors[] = 'لطفاً موقعیت واحد را انتخاب کنید.';
    if (empty($_POST['office_usage'])) $errors[] = 'لطفاً کاربری واحد را انتخاب کنید.';
    ...
    $newAd = [
        ...
        // فیلدهای اختصاصی واحد اداری
        'office_area' => $_POST['office_area'] ?? '0',
        'office_floor' => $_POST['office_floor'] ?? '0',
        'office_units_per_floor' => $_POST['office_units_per_floor'] ?? '0',
        'office_rooms' => $_POST['office_rooms'] ?? '0',
        'office_year' => $_POST['office_year'] ?? '1403',
        'office_condition' => $_POST['office_condition'] ?? '',
        'office_orientation' => $_POST['office_orientation'] ?? '',
        'office_usage' => $_POST['office_usage'] ?? '',
        'amenities' => isset($_POST['office_amenities']) ? array_values($_POST['office_amenities']) : [],
    ...
    <body style='font-family: Vazirmatn, sans-serif; text-align: center; padding: 50px; direction: rtl;'>
        <h2 style='color: green;'>✅ آگهی واحد اداری با موفقیت ثبت شد!</h2>
    ...
    <form method="POST" action="" id="propertyForm" enctype="multipart/form-data" novalidate>
        ...
        <div class="step-content" id="step2">
            ...
            <div id="step2Dynamic">
                ...
                <div class="row-half">
                    <div class="form-group"><label>تعداد واحد در طبقه</label><input type="text" class="form-input numeric-input" id="regOfficeUnitsPerFloor" name="office_units_per_floor" placeholder="۴" required></div>
        ...
        <div class="step-content" id="step5">
            ...
            <div class="form-group">
                ...
                <textarea class="form-textarea" id="regFullDesc" name="full_description" placeholder="شرح کامل امکانات و شرایط واحد اداری..." maxlength="1000" oninput="document.getElementById('charCount').innerText = this.value.length"></textarea>
...
<script>
    ...
    function updateStep2Fields(transactionType) {
        var notKeyedContainer = document.getElementById('notKeyedContainer');
    ...
    function updatePreview() {
        ...
        document.getElementById('previewContent').innerHTML = 
            ...
            '<div class="preview-card-title" style="font-size:14px;">مشخصات واحد اداری</div>' +
            specsHtml +
