...
if (($_SERVER['REQUEST_METHOD'] ?? '') === 'POST' && isset($_POST['submit_property'])) {
    ...
    // گام ۲: مشخصات اجباری تجاری
    if (empty($_POST['area_comm'])) $errors[] = 'لطفاً متراژ ملک را وارد کنید.';
    ...
    $newAd = [
        ...
        'area' => $_POST['area_comm'] ?? '0',
        'front' => $_POST['front_comm'] ?? '0',
        'flooring' => $_POST['floor_comm'] ?? '',
        'wall' => $_POST['wall_comm'] ?? '',
        'cabinet' => $_POST['cabinet_comm'] ?? '',
        'cooling' => $_POST['cooling_comm'] ?? '',
        'heating' => $_POST['heating_comm'] ?? '',
        'amenities' => isset($_POST['amenities_comm']) ? array_values($_POST['amenities_comm']) : [],
    ...
    <body style='font-family: Vazirmatn, sans-serif; text-align: center; padding: 50px; direction: rtl;'>
        <h2 style='color: green;'>✅ آگهی ملک تجاری با موفقیت ثبت شد!</h2>
...
<style>
    ...
    #vacancyGroup { display: none; }
    #vacancyGroup.visible { display: block; }
    ...
    <form method="POST" action="" id="propertyForm" enctype="multipart/form-data" novalidate>
        ...
        <div class="step-content" id="step2">
            ...
            <div id="step2Dynamic">
                ...
                <div class="row-half">
                    <div class="form-group"><label>پوشش کف</label><select class="form-select" id="regFloorComm" name="floor_comm"><option>سرامیک</option><option>موکت</option><option>سنگ</option><option>موزاییک</option><option>سیمان</option><option>کفپوش</option></select></div>
                    <div class="form-group"><label>پوشش دیوارها</label><select class="form-select" id="regWallComm" name="wall_comm"><option>کاغذ دیواری</option><option>رنگ</option><option>پنل</option><option>گچ</option><option>سرامیک</option><option>سنگ</option></select></div>
                </div>
                <div class="row-half">
                    <div class="form-group"><label>نوع کابینت</label><select class="form-select" id="regCabinetComm" name="cabinet_comm"><option>ام دی اف</option><option>هایگلاس</option><option>چوبی</option><option>فلزی</option><option>ندارد</option></select></div>
                    <div class="form-group"><label>سیستم سرمایش</label><select class="form-select" id="regCoolingComm" name="cooling_comm"><option>کولر آبی</option><option>اسپیلت</option><option>داکت اسپلیت</option><option>چیلر</option><option>پنکه سقفی</option><option>ندارد</option></select></div>
                </div>
                <div class="row-half">
                    <div class="form-group"><label>سیستم گرمایش</label><select class="form-select" id="regHeatingComm" name="heating_comm"><option>بخاری</option><option>شوفاژ</option><option>پکیج رادیاتور</option><option>ندارد</option></select></div>
                ...
                <div id="deliveryDateContainer" style="display:none;">
                    <div class="form-group"><label>تاریخ تحویل</label><input type="date" class="form-input" id="regDeliveryDate" name="delivery_date"></div>
                </div>
        ...
        <div class="step-content" id="step5">
            ...
            <div class="form-group">
                ...
                <textarea class="form-textarea" id="regFullDesc" name="full_description" placeholder="شرح کامل امکانات و شرایط ملک تجاری..." maxlength="1000" oninput="document.getElementById('charCount').innerText = this.value.length"></textarea>
